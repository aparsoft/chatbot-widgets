...
var DEFAULT_CONFIG_ENDPOINT = 'https://www.aparsoft.com/api/v1/chatbot/public/widget/{apiKey}/config/';
...
function AparsoftChatbot(_ref) {
  var apiKey = _ref.apiKey,
      ...
      configEndpoint = _ref$configEndpoint === undefined ? DEFAULT_CONFIG_ENDPOINT : _ref$configEndpoint,
  ...
  useEffect(function () {
    ...
    var handleReady = function (event) {
      var controller = (event && event.detail ? event.detail.controller : null) || window.AparsoftChatbot || null;
    ...
    var existingScript = document.querySelector('script[src="' + WIDGET_SCRIPT_URL + '"][data-aparsoft-chatbot]');
    if (existingScript) {
...
function useAparsoftChatbot() {
  ...
  return {
    getWidget: getWidget,
    openChat: function () { return getWidget() && getWidget().open ? getWidget().open() : undefined; },
    closeChat: function () { return getWidget() && getWidget().close ? getWidget().close() : undefined; },
    toggleChat: function () { return getWidget() && getWidget().toggle ? getWidget().toggle() : undefined; },
    sendMessage: function (message) { return getWidget() && getWidget().sendMessage ? getWidget().sendMessage(message) : undefined; }
  };
}
